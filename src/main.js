...
const ui = {
  ...
  current: document.querySelector('.page-current'), title: document.querySelector('.chapter__title'),
...
// Clear, softly golden morning light.
scene.background = new THREE.Color(0xdce9e5);
scene.fog = new THREE.FogExp2(0xe6dac4, compact ? 0.005 : 0.003);
...
camera.position.set(0, compact ? -.4 : 0, compact ? 19.5 : 20.5);
scene.add(camera);
...
world.position.set(compact ? -2.45 : -2.6, compact ? 1.3 : 0.25, 0);
scene.add(world);
...
function drawPhoto(ctx, x, y, w, h, story, variant, photoImage = null) {
  ...
  if(photoImage){
    // Crop uploaded images to the voyage-photo frame, then age them in-place.
    ...
    ctx.filter='sepia(72%) saturate(68%) contrast(108%) brightness(104%)';ctx.drawImage(photoImage,sx,sy,sw,sh,0,0,w,h);ctx.filter='none';ctx.fillStyle='rgba(113,76,35,.14)';ctx.fillRect(0,0,w,h);
  }else{
...
function createPageTexture(story, side, index, photoImage = null) {
  return canvasTexture((ctx, s) => {
    ...
    if (side === 'front') {
      ...
      drawPhoto(ctx,100,160,520,490,story,index+1);
      ctx.font='italic 43px Georgia'; ctx.fillStyle='#49331f'; ctx.textAlign='left'; ctx.fillText('Captain’s log',660,210);
      ...
      lines.forEach((line,i)=>ctx.fillText(line,665,290+i*62));
      ctx.strokeStyle='rgba(70,43,22,.24)'; ctx.lineWidth=2;
      lines.forEach((_,i)=>{ctx.beginPath();ctx.moveTo(665,305+i*62);ctx.lineTo(930,305+i*62);ctx.stroke()});
      ctx.fillStyle='rgba(68,43,22,.2)'; ctx.font='italic 96px Georgia'; ctx.fillText(`${index+1}`,820,820);
      drawCompassRose(ctx,785,690,90);drawTreasureRoute(ctx,s,index+2);drawJollyRoger(ctx,185,800,130,.24);
    }
...
const leftStack=roundedBox(PAGE_W-.08,PAGE_H-.08,.42,.08,pageEdge);
const rightStack=roundedBox(PAGE_W-.08,PAGE_H-.08,.48,.08,pageEdge);rightStack.position.set(PAGE_W/2,-.01,-.19);book.add(rightStack);
...
function pageMaterial(texture, side, backFace=false) {
  return new THREE.ShaderMaterial({
    uniforms:{map:{value:texture},uTurn:{value:0},uCurl:{value:.85},uReveal:{value:1},uWarmth:{value:new THREE.Color(0xffe2b0)}},
    vertexShader:pageVertex,
    fragmentShader:`uniform sampler2D map; uniform vec3 uWarmth; uniform float uReveal; varying vec2 vUv2; void main(){vec4 c=texture2D(map,vUv2);float wobble=sin(vUv2.y*91.0)*.012+sin(vUv2.x*67.0)*.009+sin((vUv2.x+vUv2.y)*143.0)*.004;float edge=smoothstep(0.0,.038+wobble,vUv2.x)*smoothstep(0.0,.041-wobble,1.0-vUv2.x)*smoothstep(0.0,.03-wobble,vUv2.y)*smoothstep(0.0,.032+wobble,1.0-vUv2.y);if(edge<.18)discard;float grain=fract(sin(dot(floor(vUv2*vec2(31.0,23.0)),vec2(12.9898,78.233)))*43758.5453);float threshold=vUv2.y*.35+grain*.65;float painted=smoothstep(threshold-.14,threshold+.08,uReveal*1.12);vec3 bare=vec3(.72,.57,.34)*(1.0-vUv2.x*.06);c.rgb=mix(bare,c.rgb,painted);c.rgb*=mix(.31,1.0,edge)*uWarmth;gl_FragColor=vec4(c.rgb,1.0);}`,
...
const introLeftGeo=new THREE.PlaneGeometry(PAGE_W,PAGE_H,24,10);
...
for(let i=0;i<12;i++){
  ...
  const leaf=new THREE.Mesh(new THREE.PlaneGeometry(PAGE_W-.1-(i%3)*.018,PAGE_H-.1-(i%4)*.014),leafMat);
  leaf.position.set(PAGE_W/2+((i%3)-1)*.008,((i%4)-1.5)*.008,.018+i*.0045);leaf.rotation.z=(i%2?1:-1)*(.0008+i*.00012);leaf.receiveShadow=true;book.add(leaf);
}

const pages=[];
for(let i=0;i<PAGE_COUNT;i++){
  // Keep the current right-hand page above the sheets that follow it.
  const pivot=new THREE.Group();pivot.position.set(0,0,.08+(PAGE_COUNT-i)*.018);book.add(pivot);
  pivot.position.y=((i%3)-1)*.009;pivot.rotation.z=(i%2?1:-1)*(.0018+i*.00025);
  const geo=new THREE.PlaneGeometry(PAGE_W,PAGE_H,36,12);geo.translate(PAGE_W/2,0,0);
  ...
  const backMat=pageMaterial(backTexture,THREE.DoubleSide,true);
...
leftStack.position.set(PAGE_W/2,-.01,-.35);coverPivot.add(leftStack);
const introLeft=new THREE.Mesh(introLeftGeo,pageMaterial(createIntroLeftTexture(),THREE.FrontSide));introLeft.position.set(PAGE_W/2,0,-.47);introLeft.rotation.y=Math.PI;introLeft.castShadow=introLeft.receiveShadow=true;coverPivot.add(introLeft);
...
const tableMat=new THREE.MeshStandardMaterial({color:0xb58b62,roughness:.84,metalness:0});
...
const mapTexture=canvasTexture((ctx,s)=>{
  const g=ctx.createRadialGradient(s*.42,s*.4,20,s*.5,s*.5,s*.76);g.addColorStop(0,'#d9c18b');g.addColorStop(.66,'#a98551');g.addColorStop(1,'#5c3b22');ctx.fillStyle=g;ctx.fillRect(0,0,s,s);noise(ctx,s,.13);
  ...
  // Small inked sea serpent for unmistakable old-world pirate cartography.
  ...
  const edge=ctx.createRadialGradient(s/2,s/2,s*.4,s/2,s/2,s*.72);edge.addColorStop(.62,'rgba(35,15,5,0)');edge.addColorStop(1,'rgba(31,12,4,.58)');ctx.fillStyle=edge;ctx.fillRect(0,0,s,s);
});
const mapMat=new THREE.MeshStandardMaterial({map:mapTexture,color:0xd9c49c,roughness:.98,side:THREE.DoubleSide});
...
createScrap('Port Royal',-4.6,2.5,4.2,2.5,-.17,'#c6ad79');
createScrap('Cape of Storms',4.8,2.7,4.5,2.7,.13,'#d2b47a');
createScrap('Admiralty Chart',-4.8,-2.2,4.1,2.35,.1,'#bfa470');
createScrap('Letter of Marque',4.7,-2.3,4.0,2.4,-.12,'#d6bd88');
...
function createCompass(){
  ...
  g.position.set(-6.25,-3.8,-.55);g.rotation.z=-.2;g.scale.setScalar(.9);return g;
}
...
function noteTexture(text){return canvasTexture((ctx,s)=>{ctx.fillStyle='#b89862';ctx.fillRect(0,0,s,s);noise(ctx,s,.12);ctx.fillStyle='rgba(54,31,15,.72)';ctx.font='italic 64px Georgia';ctx.textAlign='center';wrapText(ctx,text,70,180,s-140,78);ctx.strokeStyle='rgba(81,23,12,.65)';ctx.lineWidth=13;ctx.beginPath();ctx.moveTo(s*.7,s*.72);ctx.lineTo(s*.82,s*.84);ctx.moveTo(s*.82,s*.72);ctx.lineTo(s*.7,s*.84);ctx.stroke()},512)}
...
scene.add(new THREE.AmbientLight(0xfff4df,1.05));
scene.add(new THREE.HemisphereLight(0xe4f3f2,0xc29a72,2.25));
const key=new THREE.SpotLight(0xfff4d8,138,48,.76,.9,1);key.position.set(-9,6,16);key.target.position.set(1,0,0);key.castShadow=true;key.shadow.mapSize.set(highQuality?2048:1024,highQuality?2048:1024);key.shadow.bias=-.0002;scene.add(key,key.target);
const fill=new THREE.PointLight(0xffe8bd,42,28,2);fill.position.set(8,4,7);scene.add(fill);
const rim=new THREE.PointLight(0xcce5e3,22,26,2);rim.position.set(-8,3,6);scene.add(rim);

function addLightRay(x,y,rotation,scale){
  const geo=new THREE.CylinderGeometry(.15,2.5,14,32,1,true);
  const mat=new THREE.ShaderMaterial({transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,side:THREE.DoubleSide,uniforms:{color:{value:new THREE.Color(0xd8ad72)}},vertexShader:'varying vec2 vUv; void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',fragmentShader:'uniform vec3 color;varying vec2 vUv;void main(){float a=sin(vUv.y*3.14159)*smoothstep(0.,.18,vUv.x)*smoothstep(0.,.18,1.-vUv.x);gl_FragColor=vec4(color,a*.035);}'});
...
const dustMat=new THREE.ShaderMaterial({transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,uniforms:{uTime:{value:0}},vertexShader:'uniform float uTime;attribute float aSeed;varying float vAlpha;void main(){vec3 p=position;p.y+=sin(uTime*.18+aSeed*20.)*.5;p.x+=cos(uTime*.12+aSeed*30.)*.35;vec4 mv=modelViewMatrix*vec4(p,1.);gl_PointSize=(2.0+aSeed*3.)*(10./-mv.z);gl_Position=projectionMatrix*mv;vAlpha=.07+aSeed*.16;}',fragmentShader:'varying float vAlpha;void main(){float d=distance(gl_PointCoord,vec2(.5));float a=smoothstep(.5,0.,d)*vAlpha;gl_FragColor=vec4(1.,.88,.64,a);}'});
...
const openingState={focusX:PAGE_W/2,active:true};
...
function rightPageZ(index){return .08+(PAGE_COUNT-index)*.018}
function leftPageZ(index){return .23+index*.032}
function resetPagesForClosedBook(){
  pages.forEach((page,i)=>{setPageProgress(page,0);page.position.z=rightPageZ(i);page.renderOrder=0;page.visible=true});
...
function turnTo(next,automatic=false){
  ...
  gsap.timeline({onComplete(){page.userData.progress=target;page.renderOrder=forward?20+page.userData.index:0;currentPage=next;turning=false;updateUI();if(automatic)scheduleAutoplay();}})
    .to(state,{p:target,duration:prefersReducedMotion ? .45 : 1.55,ease:'power2.inOut',onUpdate(){
      setPageProgress(page,state.p);
    }},0)
    .to(page.position,{z:targetZ,duration:prefersReducedMotion ? .45 : 1.55,ease:'power2.inOut'},0)
    .to(camera.position,{x:compact?0:(forward?-2.1:2.1),y:compact?-.5:-.35,z:compact?13.6:12.7,duration:1.05,yoyo:true,repeat:1,ease:'sine.inOut'},0)
    .to(world.rotation,{z:forward?-.055:-.005,duration:.75,yoyo:true,repeat:1,ease:'sine.inOut'},0);
}
function enterExperience(){
  if(entered)return;entered=true;
  document.documentElement.classList.remove('book-closed');ui.enter.tabIndex=-1;ui.enter.setAttribute('aria-hidden','true');
  ...
  tl.to(camera.position,{x:0,y:compact?-.4:0,z:compact?17:16.2,duration:prefersReducedMotion ? .35 : 1.05,ease:'power2.inOut'},0)
  ...
  // 3. Open through a wide arc while the camera orbits from overhead into reading position.
    ...
    .to(world.position,{x:compact?0:.15,y:compact?1.1:.15,duration:openDuration,ease:'power2.inOut'},prefersReducedMotion ? .42 : 1.42)
    .to(openingState,{focusX:0,duration:openDuration*.85,ease:'power2.inOut'},prefersReducedMotion ? .42 : 1.42)
    .to(camera.position,{x:0,y:compact?-.45:-.25,z:compact?16.4:16.15,duration:openDuration+.25,ease:'power2.inOut'},prefersReducedMotion ? .42 : 1.42)
  ...
  // caused the cover to pass through its page block on the final quarter-turn.
    .to(coverPivot.position,{z:-.45,duration:prefersReducedMotion ? .18 : .6,ease:'back.out(1.2)'},prefersReducedMotion ? 1.12 : 3.8)
...
function closeExperience(){
  ...
  const returningPages=pages.filter(page=>page.userData.progress>.001).map(page=>({page,state:{p:page.userData.progress}}));
  const tl=gsap.timeline({defaults:{ease:'power3.inOut'},onComplete(){
    ...
    ui.enter.textContent='Reopen Book';ui.enter.tabIndex=0;ui.enter.setAttribute('aria-hidden','false');gsap.set(ui.intro,{opacity:1});
  }});
  ...
  tl
    ...
    .to(world.position,{x:compact?-2.45:-2.6,y:compact?1.3:.25,duration:closeDuration,ease:'power2.inOut'},prefersReducedMotion?.22:.42)
    .to(openingState,{focusX:PAGE_W/2,duration:closeDuration*.82,ease:'power2.inOut'},prefersReducedMotion?.22:.42)
    .to(camera.position,{x:0,y:compact?-.4:0,z:compact?17:16.2,duration:closeDuration+.12,ease:'power2.inOut'},prefersReducedMotion?.22:.42)
...
function applyPhotoToCurrentPage(photo){
  ...
  const nextTexture=createPageTexture(pageStories[currentPage],'front',currentPage,photo.image);
  ...
  document.querySelectorAll('.photo-thumb').forEach(button=>button.classList.toggle('active',button.dataset.url===photo.url));
  showToast(`Photo placed on page ${currentPage+1}`);
}
...
ui.photoInput.addEventListener('change',()=>{
  ...
  [...ui.photoInput.files].filter(file=>file.type.startsWith('image/')).forEach(file=>{
    const url=URL.createObjectURL(file);const image=new Image();
    image.onload=()=>{const photo={url,image,name:file.name};uploadedPhotos.push(photo);addPhotoThumbnail(photo);applyPhotoToCurrentPage(photo)};
    image.onerror=()=>{URL.revokeObjectURL(url);showToast(`Could not read ${file.name}`)};image.src=url;
  });
  ui.photoInput.value='';
});
...
addEventListener('keydown',e=>{if(!entered&&['Enter',' '].includes(e.key))enterExperience();else if(entered&&e.key==='Escape')closeExperience();else if(e.key==='ArrowRight')turnTo(currentPage+1);else if(e.key==='ArrowLeft')turnTo(currentPage-1)});
...
canvas.addEventListener('pointerup',e=>{if(dragStart!==null&&Math.abs(e.clientX-dragStart)>45)turnTo(currentPage+(e.clientX<dragStart?1:-1));dragStart=null});
...
gsap.timeline({delay:.65})
  ...
  .from('.masthead',{opacity:0,y:-12,duration:1},.55)
  .from('.intro > *',{opacity:0,y:22,duration:1,stagger:.12,ease:'power3.out'},.55);
...
function animate(){
  ...
  lookTarget.set(world.position.x+openingState.focusX+px,world.position.y-py,0);
  ...
  if(compass?.userData.needle){
    ...
    compass.position.z=-.55+Math.sin(t*18.5)*.018;
  }
